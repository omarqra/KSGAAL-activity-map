...
export const metadata: Metadata = {
  title: "تسجيل الدخول",
  description: "وحدة التحكم — تسجيل الدخول",
};
...
export default async function LoginPage({ params }: { params: LocaleParams }) {
  ...
  if (session) {
    ...
    // Stale/invalid session (deleted user, inactive, malformed sub):
    // fall through and render the login form. The cookie will be
    // overwritten when the user signs in via the Server Action.
  }
    ...
    >
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-md flex-col items-center px-6 pt-6 pb-10 sm:pt-10">
        <h1 className="text-center text-[28px] leading-tight font-bold text-[#082F18] sm:text-[34px]">
          {isAr ? "مجمع الملك سلمان العالمي للغة العربية" : "King Salman Global Academy for Arabic Language"}
        </h1>
        <p className="mt-2 max-w-sm text-center text-[13.5px] leading-relaxed text-[#6b6256]">
          {isAr
            ? "سجّل دخولك للوصول إلى لوحة إدارة الأنشطة والمنظمات والدول."
            : "Sign in to manage activities, organizations and countries."}
        </p>
        ...
        <div className="mt-4 flex items-center gap-2 text-[11.5px] text-[#6b6256]">
          <ShieldCheck className="h-3.5 w-3.5 text-[#082F18]" />
          <span>
            {isAr
              ? "اتصال آمن ومُشفّر · بيانات الدخول لا تُسجَّل"
              : "Secure encrypted connection · credentials are not logged"}
          </span>
        </div>
      </section>
